...
use crate::constant::{CorpCardData, APPLICATIONS_URL, AUTUMN_ORDER_CORP_INFO, DISCORD_URL};
use crate::model::stats::StatsDto;

async fn get_stats() -> Result<Vec<StatsDto>, reqwest::Error> {
    let api_url = match option_env!("API_URL") {
        Some(url) if !url.is_empty() => url,
        _ => "http://localhost:8000",
    };

    let api_stats_url = format!("{}/stats", api_url);

    let req = reqwest::Client::new().get(api_stats_url).send().await?;
    let stats: Vec<StatsDto> = match req.json().await {
        Ok(stats) => stats,
        Err(err) => {
            tracing::error!(" Failed to deserialize stats to JSON: {:?}", err);
...
pub fn CallToAction() -> Element {
    ...
    let mut chart_stats = use_signal(Vec::<StatsDto>::new);
    ...
    use_effect(move || match &*future.read_unchecked() {
        Some(Ok(stats_data)) => {
            ...
            if stats_data.len() == 1 {
                stats_data.insert(0, StatsDto::default());
            }

            chart_stats.set(stats_data.to_vec());
        }
        Some(Err(err)) => {
            tracing::error!("Failed to fetch corporation stats due to error: {:?}", err)
        }
        None => (),
    });
                ...
                div { class: "w-full flex flex-col items-center xl:w-1/2",
                    ...
                    div { class: "card card-compact h-64 sm:h-96 w-full max-w-[696px] shadow",
                        ...
                        LineChart {
                            ...
                            series: vec![chart_stats.iter().map(|stat| stat.member_count as f32).collect::<Vec<f32>>()],
                            labels: chart_stats.iter().map(|stat| stat.date.format("%Y-%m-%d").to_string()).collect::<Vec<String>>()
                        }
                    }
                }
